...
- renamed filenames both commands use to save their data
 - InvokeAddinCommand - %appdata%/revit-ballet/runtime/InvokeAddinCommand-last-dll-path
 - InvokeLastAddinCommand - %appdata%/revit-ballet/runtime/InvokeAddinCommand-last-command
...
added check to InvokeAddinCommand to prevent this command from ever being saved to %appdata%/revit-ballet/runtime 
**** =18:51= refactoring
...
**** =17:30= LogViewChanges SwitchViewByTitle
 ...
 LogViewChanges was modified to remove all entries from %appdata%/revit-ballet/runtime/LogViewChanges OnStartup
...
if accidentally a view gets removed in another way, they can be manually removed in %appdata%/revit-ballet/runtime/LogViewChanges
...
** May
*** 11
**** =22:55= very disappointingly it's not possible to change graphics settings of revit links via api [[revit link display settings are not available via api][https://forums.autodesk.com/t5/revit-ideas/api-for-rvt-link-display-settings/idc-p/13624379]]
...
*** 22
**** =13:56= setting out plans seem to be automatable
could be approached as generating different types of chains of dimensions
1. segments along walls split at openings (windows/doors)
2. segments between selected elements
*** 23
**** =21:59= ReferenceIntersector technique in revit aka ray casting requires an opened 3d view
that is very annoying, requiring user to have a 3d view opened or having to wait for it to open is a burden
trying to perform measurements with a self-grown version of ray tracing
[[autodesk docs][https://help.autodesk.com/view/RVT/2024/ESP/?guid=Revit_API_Revit_API_Developers_Guide_Revit_Geometric_Elements_Geometry_Finding_geometry_by_ray_projection_html]]
*** 24
**** =12:00= making FilterDoorsPartM
***** =13:00= improved performance of adjacent wall selections to "usable"
collected all corresponding walls of about 1000 doors in 18 seconds
****** =14:00= gemini seems to have got it down even more, but not all walls were collected
suspicious, but worth investigating
***** =16:30= first time I got dimensions drawn via revit api
starting to hit ai limits on major platforms, many difficult problems are cropping up
drawing dimensions proves, as somewhat expected, to be a challenge and ai isn't great at finding geometric reference planes
***** =16:37= managed to get dimensions snap in the right places for doors and walls
***** =17:58= dimensions now snap correctly on all sides and corresponding columns are shown in datagrid
***** =23:40= because of the size of the file progress became difficult
on the last breath managed to generate code that irons out a few edge cases I noticed
tried refactoring code into 4 separate files, which did help speed up maintenance
*** 25
**** =13:00= making FilterDoorsPartM
***** =13:42= ironed out remaining issues with wall selection
surprisingly speed has improved at some point, now able to perform these calculations on 400 doors through various floors only in 10 seconds
run out of claude credits in no time, finished fixes with gemini
***** =14:25= removed debugging code and sorted positions of dimensions
seemingly getting close to finishing it
***** =14:31= doubling searchDistance from 0.5 to 1.0 didn't have much negative impact on performance
for 1000 doors changed only from 22.5s to 25.5s
***** =19:32= progress is steady although slow and highly dependent on claude
claude proves to be the only model to comprehend tasks at hand
still need to add more robust optional diagnostics as expecting future complications in various rules in wallFinding logic
also need to find a way to designate "push" and "pull" sides of door
***** =22:06= not sure how to code door orientiation
most other coding seems to be complete or close to being complete now
****** =22:45= the way it was coded already accounts for door orientation
[[https://raw.githubusercontent.com/baleti/revit-scripts-docs/refs/heads/main/FilterDoorsWallOffsets-Door-Orientations.png]]
orientation understood as combinations of HandFlips and FacingFlips
it doesn't stop the family from setting it differently
  that's why I called them Left, Right, Left Reverse, Right Reverse rather than leading/following edge, push/pull side, key side
  there doesn't seem to be a reliable way to ensure that Left side will always correspond to nib of leading edge and left reverse to nib of following edge
  it appears to be the case in most instances, but likely need to be checked on a family by family basis
besides "local orientation" of doors architecture likely has to also account for direction of travel to establish orientation (handing: left/right/left-reverse/right-reverse)
command also isn't trying to account for door frame thickness as there doesn't seem to be a reliable way to code it
  revit door usually also seem to set "temporary  dimensions" to door opening rather than trying to snap to user-made door frame 
***** =23:25= spotted one situation where a wall wasn't correctly excluded as occluded
overall it was an exception, prevailing majority of selected door/wall situations were measured correctly
maybe I will address it later (if manage to reproduce)
** June
*** 27
**** =19:00= thinking about graph structure of buildings
:PROPERTIES:
:ID:       91860881-9034-4fff-aebd-8d4a0823f548
:END:
***** graph comes up for the third time
first when enumerating communal doors in hallways
second when establishing direction (inside/outside orientation) of apartment doors
now when assigning numbers to apartments
***** assigning numbers to apartments
****** custom logic
clockwise
from left to right
both for each block and for each lobby
****** automating this custom logic requires some global knowledge of the building
what apartment does a selected room belong to
what lobby does a selected apartment belong to
what block does a selected lobby belong to
****** implementation may be similar to previous commands
SetInstanceParameterToNumberSequenceAlongX
SetInstanceParameterToNumberSequenceClockwise
command needs to build necessary portion of a graph and assess selected room accordingly
****** =19:43= can't think of a way to distinguish communal hallways
issue may arise in internal hallways (in apartments)
can't think of a standard way to designate such spaces
******* need a generalized way to distinguish room types
need to "enrich" the graph
******* gave up
going to look into streamlining existing selection methods instead
****** =19:53= perhaps can't get around this by designating hallways manually
this may still be a simple operation
hallways acting as a "pivot" for the command
may be stored in a separate location to avoid polluting project parameters
******* =20:00= this does start to feel overconstrained already
a lot of assumptions are being made that may break
will apartments be always connected to lobbies
won't topology of hallways drastically change in other projects
is it safe to assume positions of flat entrance doors dictate order
******* =22:37= a simple place to store temporary graph nodes are selection sets
e.g. "hallways" and "entrances"
****** =20:13= strategy as previously mentioned may be building a graph first
separate from project parameters
this exercise would require hallways and entrances
algorithm would then compare distances between selected rooms and hallways and hallways to entrances
shortest distances should answer what rooms belong to what hallways and what hallways belong to what blocks
****** I find it surprising that shortest path designates relationship
it brings me back to reading about principle of least action in physics
something philosophical about conservation of energy and symmetries of the universe
****** =20:35= if shortest path designates relationship graph may not be necessary
why not use cartesian distance instead
****** =00:34= assuming that if elements of stair category are in rooms, these rooms will have a vertical connection
this simplifies room graph creation (previously assumed that a selection set with stair rooms will be necessary)
hopefully other vertical circulation rooms won't be necessary
****** [2025-06-28 Sat 16:15] an intermediate representation/interface of a graph should be provided
******* simplicity is important, engineering schematics may be a good reference
graph will likely be broken in many places and manual and graphical ways of editing it would be useful
like an electronic schematic
******* think for example of git tree, dynamo nodes, gephi
potential opportunity for exciting graphic representations of the building
******* generating this graph is likely bound to be a slow process
so try caching it and displaying asynchronously in a separate view as it generates
****** [2025-06-28 Sat 16:15] inheriting data from areas sounds like a good idea after all
this seems congruent to what "revit expects"
******* this ensures that whatever "naming" used will be retained
wouldn't require any new schema/parameters/state
******* if area not available then clean it up via a "schematic view"
****** =21:31= realized that graph of areas may be a better approach than graph of rooms
areas are the guiding principle, while rooms (and all other elements for that matter) should follow them
depending on revit model quality however, both areas and room may need to be used
****** [2025-06-29 Sun 13:11] "scene graph" appears to be standard terms
standard TreeView in winforms may be sufficient to represent them
alternatively a plain text or datagrid with a column for parent id
******* bear in mind that they may not always for a DAG
cycles may appear, perhaps then a second column for second link/parent ?
in may DAG is a scene graph, by a "dependency graph" can have cycles
****** [2025-07-01 Tue 13:04] will likely need to sort the graph topologically
this requires the graph to be a DAG
not sure if that can be ensured or if an alternative approach will be needed
******* an interesting fact, topological sort is like a recording of depth-first traversal
depth first traversal uses a stack data structure so I expect it to be fast
****** =18:10= thinking about 'representational equivalence' of tables, graphs, linear systems
finding out that org-mode tables also adopted $ (dollar sign) for designating columns made me think if table parameters and variables in programming have some useful connection with nodes in a graph
concluded though that any table is just some view and can be rearranged arbitrarily
graph represenations varies between adjacency matrix (dense graphs) and adjacency list (sparse graphs) depending on need
  nothing forces parameters/variables to be in columns
  in fact parameters/variables/nodes and both rows and columns seem to represent the same concept, that of a variable
    expressions, edges, grids represent relationships between them
****** =18:47= wondering why not simply represent areas in a table and represent each topological position of every area by an array of "parents" and "children" in separate columns
possibly is graph turns out not to a directed acyclic graph these could be changed from 'parents' and 'children' columns to a single 'neighbours' column?
  directed acyclic graph needed for topological sorting
because schema isn't fixed you could even generate separate columns (e.g. parent 1, parent 2, etc.) dynamically as much as needed
that's the simplest and most boring solution
  other solutions may always follow later
this representation seems equivalent to the popular 'adjacency list'
  nodes are items rather than columns
****** =20:00= finally settled on thinking about graphs as set of objects and edges as their properties
rather than treating edges as separate items
most real world applications seem to approach them this way as well
  social networks, electric circuits
  that seems to logically map to an adjacency list/matrix
    opens up possibility to higher-level analysis like clustering, connectivity and other patterns
  edges as a currency of graph nodes
******* as mentioned before I could therefore retain previous tabular representation (datagrid) and simply treat connections/edges as another column
****** correlation of this graph-based approach to the way mechanical, electrical and plumbing systems are designed
[2025-07-04 Fri 12:09]
also graphs, schematics, parameteric by necessity
****** =15:45= an elegant concept that captures many of these ideas is "configuration space"
"set of all possible positions the system can be in"
"for a mechanical system, it's described by its degrees of freedom"
manifolds describe properties of coordinate systems ("spaces") themselves
topology, differentiable geometry, mechanical systems degrees of freedom
******* simplical complexes
"simplex" is a generalization of a triangle in more dimensions
  2-simplex is a triangle, 3 vertices
  3-simplex is a tetrahedron, 4 vertices
said to be useful in modelling neural networks
  because a "complete graph" aka graph where all nodes are connected to each other aka fully connected layer in neural networks is also a simplex of degree number of nodes
  any graph can be interpreted as a simplicial complex aka combination of simplices of varying dimensions
simplicial complexes are a branch of algebraic topology
their applications include analysis of neural network and other high dimensional graph (e.g. image analysis)
  topological patterns have been used to analyze functional patterns in the blue brain project
****** one potential application of these mathematical concepts would be data analysis of patterns between various possible graphs made between building areas
[2025-07-05 Sat 13:06]
thinking of x,y,z positions of area centroids just as another property
rather than its 3d embedding
therefore maybe other properties can be used in the same manner (area, circulation links, use class)
****** =14:04= notion of abstractness of graph theory and topology over continuous geometry
****** multi-column tree view as a representation for graph-table data structure
[2025-07-07 Mon 10:48]
it combines benefits of simplifity of tabes with richness of graphs/trees
******* how will it know on which column to expand when different columns can expand to different subtables?
*** 28
**** =16:48= considering caching machanisms for tables from FilterSelectedElements
strategy would be to subscribe to some events from revit and refresh the table only when selected elements have been changed
a caching mode should be added to force refresh on suspicion of cache incoherency
events for view updates used for example by SwitchViews command work in ~99% cases, but that sometimes may not be enough
also this might allow for faster rebuiliding of the tables (only rebuild elements that changed)
**** =18:49= testing changes made by SelectionModeManager appears to be successful
***** tested commands
SelectViews, RenameInstanceParameters, FilterTags, FilterText, RenameGroups, ListViewsByDetailItemSelected, FilterPosition, FilterDoors, FilterSheets, RenameSelectedTexts, SelectAssociatedDimensions, SelectAssociatedTagsOfSelectedElements
***** inexperience with git --amend command threw me off-guard in the middle of applying these complex changes
I missed to observe SelectionModeManager.cs being removed upstream
afterwards could only take modified history on trust
****** later on was able to repeat this process and discover that git push --force "orphans" previous branch
this reassured me that commits can be fairly flexibly rewritten (at a risk of 'rewriting history')
this very journal entry has been amended two times in git history at least
******* among other methods, on github "sibling" commits can be noticed by having the same parent
for example: [[https://github.com/baleti/revit-scripts/commit/6213940238fd025598ff6e9ab628d839faaaf416][main]] and an [[https://github.com/baleti/revit-scripts/commit/6417db923afd574d45f540dd320c5461052a6b07][orphaned sibling]]
******* therefore previous branch is still available to see on github
  in this case it was: https://github.com/baleti/revit-scripts/commit/b976e20
******* orphaned branches can be listed ~git fsck --unreachable --no-reflogs~
an overcomplicated version that sorts by date and shows pretty colors
#+begin_src bash
git fsck --unreachable --no-reflogs | grep 'unreachable commit' | awk '{print $3}' | while read hash; do   printf "%s|%s\n" "$(git show --no-patch --format="%ad" --date=iso "$hash")" "$hash"; done | sort | cut -d'|' -f2 | while read hash; do   git show --no-patch --format="%C(yellow)%h%C(reset) %C(cyan)%ad%C(reset) %C(green)%an%C(reset): %s" --date=iso "$hash"; done
#+end_src
** July
*** 02
**** =00:01= loading addin into revit 2019 yielded a lot of errors
start setting up solution for "multi-version build"
needs to be completely automated, minimum maintenance
***** think about testing
*** 06
**** =15:58= hit sad limitation of revit api, there is no way to unhide in view elements from linked models
:PROPERTIES:
:ID:       c4a24ec8-e379-4ebf-b0c4-bed868fe87f2
:END:
given that temporary isolate methods don't work on elements on linked models and other limitations like filters not distinguishing between linked models working with linked models in revit becomes is severely limiting
[[https://forums.autodesk.com/t5/revit-ideas/unhide-elements-in-view-in-linked-models/idi-p/13712780][revit idea]]
***** possible workaround would be duplicating the view precisely and performing a "sacrificial" hide in view there
*** 12
**** =14:43= considered using Fody.Costura to embed code from references .dlls into a single .dll file
sadly all such tools like fody.costura or ilmerge don't perform dead code elimination and embed entire .dll
  this seems to lead to noticeable waste that I'd rather avoid by simply shipping other dependencies as separate files
  perhaps it would not benefit from other optimizations offered by windows when .dlls are referenced as separate files
  shame this isn't available in the default toolchain build process
also don't know how addin worked before if I only shipped a single .dll file without its references
  perhaps they were cached globally somehow
*** 30
**** =14:00= ExportSelectedViewsToPDF command turned out to be ~25% slower than revit builtin PDF Exporter
ExportSelectedViewsToPDF - 12 minutes
Revit UI - 9 minutes
*** 31
**** =12:30= caught myself [[id:c4a24ec8-e379-4ebf-b0c4-bed868fe87f2][again]] not remembering that revit api doesn't let you hide/unhide elements in linked models
** August
*** 01
**** =20:48= struggling a lot to get SelectSameInstancesInGroups to work
this breaks when a group has excluded members
cannot use location of elements, because group transformation cannot be obtained
could obtain location but comparing corresponding elements
but when elements are excluded there is no way to obtain corresponding elements
in summary revit api has really screwed up groups
https://forums.autodesk.com/t5/revit-api-forum/groups-check-for-mirrored/td-p/5868527
***** new strategy
if group doesn't have excldued members use default indexing
if group has excluded members reconstruct transformation
  tried principal components analysis but it didn't seem to work
*** 03
**** =21:39= unable to finish commands adding elements to model groups
in summary revit api is completely nerfed when it comes to group
  feels deliberate
no way to add to existing groups, only ungroup and regroup, but that changes location of origin
  this makes it impossible to later swap old groups with new ones
  no way to change origin...
***** =00:55= a potential hack how to solve this limitation struck me while writing this paragraph
I haven't quite finished writing it, but more importantly potentially adding extra element (e.g. model lines) to shift group origin to land in the same place as the source group could solve it
****** along with recently written CopySelectedElementsAlongContainingElements command that seems to be able to reverse engineer transformation should provide a fighting chance against revit groups
*** 05
**** =15:40= while developing commands automating placements of sprinklers from suppliers cought myself struggling to make precise selections of elements within groups
it seemed simplest to rely on rooms as logical, easily manageable groupings
however usage of rooms can be inconsistent and lead to unexpected errors
***** thus wondering if it could be recomputed from scratch based on arrangement of architectural elements rather than purely by user defined spaces
likely combination of both
graph theory
***** related commands
FilterSelectedElementsByContainingGroupsWithRooms
CopySelectedElementsAlongContainingGroupsByRooms
*** 06
**** =00:00= coming to conclusion that it is likely beneficial to use builtin room and area types in revit to construct and parametrize building graph/topology
they are user friendly, easily debuggable and versatile
fundamentally I want to believe that they were meant to be used for this type of analysis
  just wasn't meant to be
not sure if custom parameters will be available
  may need to load it all into Comments parameter or similar
**** =14:40= finding myself too often repeat these instructions in prompts
write a c# 7.3 revit api 2024 command called <name>
make it obtain current selection using var selectedIds = uidoc.GetSelectionIds(); (implemented already in other .cs files)
make it display user a datagrid to choose workset to change using this custom code (implemented already in other .cs files):
...
*** 09
**** =19:31= thinking of [[id:91860881-9034-4fff-aebd-8d4a0823f548][past explorations]] of graphs and topologies simply as an exercise in sorting and grouping of rooms
***** examples where sorting of room was needed
****** numbering apartments
apartment can be thought of as a higher-level concept derived from certain grouping of rooms
****** numbering doors
rooms within communal spaces need to have consistent order
communal space/area again as a higher-level concept dervied from a certain grouping of rooms
******* commands
SetInstanceParameterToNumberSequenceAlongX
SetInstanceParameterToNumberSequenceAlongXWithPadding
****** assigning sprinklers to flat type and core groups
sprinkler elements have been modeled only in certain typical flat layouts and requrie replicating across other spaces represented as revit groups
grouping of rooms was complex and required many heuristic rules to ascertain that any one elements is assigned to a correct group
******* commands
FilterSelectedElementsByContainingGroupsWithRooms
CopySelectedElementsAlongContainingGroupsByRooms
*** 10
**** =00:29= little progress in getting CopySelectedElementsAlongContainingGroupsByRooms command to work
it became long and complex and tests are slow
determined to get it to work, although investment isn't likely to be returned at this point
*** 16
**** =22:09= CopySelectedElementsAlongContainingGroupsByRooms approaches completion
subject to tests
** November
*** 10
**** =11:00= welcome back
"completed" work on autocad-ballet and returning to revit-ballet development
  on occasion of switching employment
***** applied lessons learned from autocad-ballet
dotnet sdk 8 workflow
claude code (ai agent) workflow
documentation... is coming
**** =12:00= programatic control over shortcuts
suspecting that shortcuts can be adjusted without user intervention
changing KeyboardShortcuts.xml file appears to be loaded by revit
  %appdata%\Autodesk\Revit\Autodesk Revit <YYYY>
need to be complete set of shortcuts, revit by default won't merge only new ones, it will delete any missing ones in that file
that file only appears when user makes changes to shortcuts, many users may not have that file
***** sadly couldn't find a KeyboardShortcuts.xml prototype installed with revit
will have to resort to shipping a complete KeyboardShortcuts from 
** November
*** 17
**** =23:50= discovered that default keyboard shortcuts in revit 2024 are hard coded in to utility.dll that lives in program files/autodesk/revit 2024
if I manage to understand how these string literals correspond to commands I may have a more reliable way to avoid clashes with default shortcuts in case user haven't ever customized them and doesn't have a KeyboardShortcuts.xml file in %appdata% revit directory
however it may be easier to somehow just collect default keyboardshortcuts from various revit installations, this reading of dll is likely brittle and changes across revit versions
shame revit doesn't have a simpler mechanism just to "add" new shortcuts (although this may simply stem from mistrust to addin vendors)
***** UIFramework.dll appears to have code assembling shortcuts
[2025-11-18 Tue 14:35]
fortunately it's a .net binary, so will be much easier to analyze that native Utility.dll
***** =14:59= may have just hit gold in "/UIFramework;component/KeyboardShortcut/PredefinedShortcuts/ShortcutsOneBox.xaml"
***** =21:30= this turned out to only show about a half of shortcuts exported by revit
this likely marks the end of this idea
relying on user having modified shortcuts is too brittle
so is trying to maintain your own copies of complete keyboardshortcuts.xml
*** 20
**** =00:25= while working on the installer accidentally have deleted entire codebase and runtime folder
uninstaller.exe correctly deleted revit-ballet folder, which double up as my codebase (similarly to autocad-ballet)
thanks to recent push to git remote not much was lost, runtime folder is completely gone, but it likely contained only transient information
**** =12:58= planning how to set up testing
***** add tests folder
***** windows virtual machine
****** all revit versions installed
****** sshd listening on vsock
virtual machine should run offline (no network interfaces)
****** btrfs
***** invocation script on hypervisor
****** btrfs snapshot qcow2 disk for clear environment
****** copy addin into qcow2 disk
***** revit script on guest vm
****** start revit with journal files
****** collect results
some results may show up in journal files generated by this revit session
   depending on whats being tested
*** 25
**** =11:59= possibly solved issue of programmatic setup of revit shortcuts
***** summary
****** remove redundant attributes and elements without "Shortcuts" attribute set
xmlstarlet ed -L -d "//ShortcutItem[not(@Shortcuts)]" -d "//@CommandName" -d "//@Paths" KeyboardShortcuts.xml
****** isolate customizations
removes the same lines comparing it to the default
grep -Fxvf KeyboardShortcuts-2024.xml KeyboardShortcuts-custom.xml
***** key takeaway
****** shortcuts can be set by a file in %appdata%\Autodesk\Revit\Autodesk Revit <YYYY>
KeyboardShortcuts.xml
****** only CommandId and Shortcuts attributes are needed
Paths and CommandName attributes are not needed and depend on language
***** design
installer should combine KeyboardShortcuts-custom.xml with corresponding versions of each year at build time
custom script should run on schedule updating KeyboardShortcuts-custom.xml
  or ideally whenever I change it
***** obtain default shortcuts for revit versions
was able to obtain them after finishing setup of qemu virtual machine for testing across all supported versions (2017-2026)
turned out that there
they showed
***** solved concerns over localization
testing showed that removing localized attributes (Paths and CommandName) didn't cause issues 
if that's correct moving forward I will need to decide how to make changes to shortcuts while keeping them synchronized with production revit
likely via an xml file and then running installer to update them
or by running a script that updates xml file after making changes to shortcuts in production revit
*** 30
**** =22:28= testing of ui in revit likely can't be done from addin alone
instead a separate server should be created outside of revit running on windows host
it should perform all the actions as if it was the user
possibly through windows UI automation or other api
***** possible solution may be making ui modeless
not sure if it won't break any functionality
i may have tried it before already and ended up reverting to modal (not sure)
***** openssh server on windows should be a perfect solution to drive ui automation
i knew it, i knew it will be useful
sounds like a more univeral and more robust solution
** December
*** 01
**** =01:39= discovered that multi-document (aka InSession) scope commands work in revit api
was able to rename sheet numbers and create walls in documents opened in a single revit process without switching active document
this is very satisfying as it translates directly a lot of techniques developed in autocad-ballet where -in-document and -in-session commands wer heavily used
***** on a less happy note tls messages in named pipe relay on my development envrionment started failing especially when i was testing this multi-write commands for some reason
#+begin_src sh
curl --insecure -X POST https://127.0.0.1:23717/roslyn  -H "X-Auth-Token: $(cat revit-ballet/runtime/network/token)" -d 'Console.WriteLine("Hello from Revit!");' | jq
curl: (35) TLS connect error: error:0A000126:SSL routines::unexpected eof while reading
#+end_src
unlikely that I will be able to resolve it, possibly it's going to resolve itself or stop being noticable after I restart windows, but more likely it will require me to switch wsl networking from nat to mirrored
hopefully it won't cause any problems with it admins
*** 21
**** =16:39= finished initial refectoring of commands into InDocument and InSession scopes
amazed that this is supported in almost exactly the same way as in autocad
fitting all shortcuts becomes an issue, may need to redesign it later
***** consider your own shortcuts mechanism like emacs with a modifier key starting sequence
another interface to InvokeAddinCommand effectively
if performance will be acceptable
* 2026
** January
*** 11
**** =21:56= assigning new shortcuts proves increasingly difficult due to revit design
existing shortcuts block new ones
e.g. shortcut ts blocks tsa, because revit will wait after ts is typed for the next character
autocad in that sense was better, because it required explicit confirmation with spacebar when shortcuts was complete
but can't think of a proper way to solve it
*** 18
**** =23:00= moved ai agents development environment from wsl/qemu to hyper-v
noticeable performance improvement
*** 19
**** =20:00= testing pre-compiled server endpoints to improve performance of InNetwork commands
*** 24
**** =18:39= struggling with smb and git incompatibilities
case insensitivity and lack of symlinks cause a lot of errors and eventually confused claude agent leading it to lose substantial portion of codebase
I partially saw it coming and started refactoring recent changes before the incident, however it still took down a substantial portion of changes (changes made for InvokeAddinCommandInNetwork and other InNetwork commands)
***** =20:58= shot myself in the foot this time by running
git stash push -u -m "temp"
cd ..
git clone git@github.com:baleti/revit-ballet.git revit-ballet-clean
cd revit-ballet-clean
git rebase --exec 'git commit --amend --no-edit -S' 21d80163c
cd ..
rm -rf revit-ballet/.git
cp -ra revit-ballet-clean/.git revit-ballet
this removed remaining untracked changes that survived previous incident
lol, there is some comedic irony in this situation that I may see later
** April
*** 18
**** =18:28= returning after a longer hiatus, had to finish architect exams (part 3)
it had its moments, but overall disheartening, no realistic strategy to succeed commercially, very defensive (cost-based pricing)
**** =18:30= changing folder structure and decentralizing repository
effectively finishing what I started with worktrees
***** removing deploy and runtime folders
setting up a testing windows machine where deployments can be uploaded and tested in an isolated environment
***** each machine will have a separate git repository rather than 
it still pays off to use worktrees if multiple agents will work at the same time in the same machine
i won't have to prepand branch names with hostname anymore as it will be self-evident
  this should prove easier to track
***** can then move git repo out of %appdata%/revit-ballet folder
may place it anywhere, for convenience placing them in ~/revit-ballet
moved runtime files from %appdata%/revit-ballet/runtime to %appdata%/revit-ballet
*** 19
**** =18:32= looking into memory system of claude code
not happy about tying yourself this closely to proprietary system, but may have to
need a solution to provide context that will get loaded on each start, but not commited to git, as well as context that gets loaded in every project
come to think of it, this may as well be achieved with combinations of .gitignore rules and symlinks
I guess we may depart from it in the future then, for now claude code stays as the main coding agent, so may as well follow its practices
  bearing in mind that we can depart from them later if needed
***** hierarchy
****** commited to remote
./CLAUDE.md
****** not commited to remote
./CLAUDE.local.md
****** not commited to remote, shared across all projects
~/.claude/CLAUDE.md
***** =18:46= a more portable approach however may be to explicitely specify in standard CLAUDE.md to read files in .claude directory
or elsewhere
rather than tying yourself to a rigid naming structure
***** =19:45= decided to rely on claude's memory mechanism instead
****** completely separate from CLAUDE.md
it should keep it clean 
other developers may have other methods for deploying/debugging
******* also injects context at startup
but only a reduced portion, details can be read on demand
******* shared across worktrees
claude code recognizes folders that are worktrees and matches memories accordingly
****** funny enough, that's what i started with
****** another advantage is that it will be easier to turn it into a share for all virtual machines
just because its' a folder rather than a single file
****** a disadvantage is that it ties to claude code's proprietary mechanism
however pi agent (for example) seemed to support this solution as well
****** =20:03= there appear to be annoying bugs that block usage of memories across worktrees
look trivial to solve, hopefully soon
bug report: https://github.com/anthropics/claude-code/issues/34437
apparently it doesn't affect worktrees nested in root git folder
  hopefully this small design decision may spare me the trouble
******* =20:28= good news, memory system does appear to be working with worktrees as intended
confirmed it through structure of ~/.claude/projects and checked memories loaded by claude code instance in a worktree
assuming that agent didn't lie about it, there doesn't seem to be a reliable way to test it as it's a (practically) proprietary code
******** per-worktree folders in ~/.claude/projects store conversation history, not memories
these are stored only per-git-repo
