...
public class Resize3DSectionBox : IExternalCommand
{
        ...
        ElementSet elements)
    {
        ...
        // Calculate the new extents by scaling the section box
        XYZ newExtents = (sectionBox.Max - sectionBox.Min) * scaleFactor / 2;

        // Set the new min and max points for the section box
        sectionBox.Min = center - newExtents;
        sectionBox.Max = center + newExtents;

        using (Transaction tx = new Transaction(doc, "Resize 3D Section Box"))
        {
            tx.Start();
