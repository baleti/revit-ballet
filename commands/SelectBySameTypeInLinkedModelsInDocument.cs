...
public class SelectBySameTypeInLinkedModelsInDocument : IExternalCommand
{
    ...
    private string GetElementTypeSignature(Element element)
    {
        try
        {
            ...
            if (element.Category != null)
            {
                ...
                if (element.Category.Id.AsLong() == (int)BuiltInCategory.OST_TextNotes)
                {
                ...
                else if (element.Category.Id.AsLong() == (int)BuiltInCategory.OST_Dimensions)
                {
                    var styleParam = element.get_Parameter(BuiltInParameter.ELEM_TYPE_PARAM);
                    if (styleParam != null)
                    {
