...
public class RehostToJoinedWalls : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        foreach (ElementId id in selectedIds)
        {
            ...
            {
                TaskDialog.Show("Warning", $"Element id {fi.Id.AsLong()} does not have a valid host wall. Skipping.");
                continue;
            }
        ...
        using (Transaction trans = new Transaction(doc, "Rehost Elements"))
        {
            ...
            foreach (FamilyInstance fi in validHostElements)
            {
                ...
                if (locPoint == null)
                {
                    TaskDialog.Show("Warning", $"Element id {fi.Id.AsLong()} does not have a point location. Skipping.");
                    skippedElements.Add(fi);
                    continue;
                }
                ...
                if (level == null)
                {
                    TaskDialog.Show("Warning", $"Could not determine the level for element id {fi.Id.AsLong()}. Skipping.");
                    skippedElements.Add(fi);
                    continue;
                }
