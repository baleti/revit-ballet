...
public class SwitchToLastPhase : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        string logFolderPath = Path.Combine(appDataPath, "revit-ballet", "runtime", "LogPhaseChanges");
        ...
        if (matchingEntries.Count >= 2)
        {
            ...
            chosenPhase = doc.GetElement(((long)secondLastEntry.phaseId).ToElementId()) as Phase;
            if (chosenPhase == null)
            {
            ...
            string currentLogLine = $"{currentPhase.Id.AsLong()} {FormatString(currentPhase.Name)} {FormatString(activeView.Name)}";
            // Second, log the chosen phase (the one we just switched to).
            string chosenLogLine = $"{chosenPhase.Id.AsLong()} {FormatString(chosenPhase.Name)} {FormatString(activeView.Name)}";
        ...
        else if (matchingEntries.Count == 1)
        {
            ...
            chosenPhase = doc.GetElement(((long)entry.phaseId).ToElementId()) as Phase;
            if (chosenPhase == null)
            {
                message = "Invalid phase found in the log.";
                return Result.Failed;
