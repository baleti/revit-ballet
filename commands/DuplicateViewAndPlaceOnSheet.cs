...
namespace RevitAddin
{
    ...
    /// <summary>
    /// Command that duplicates views and places them on sheets.
    /// Uses views from selection or active view.
    /// If sheets are selected, uses them; otherwise shows DataGrid to select target sheets.
    ...
    public class DuplicateViewAndPlaceOnSheet : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            // ─────────────────────────────────────────────────────────────
            // 1. Collect views using InputResolver, sheets manually
            // ─────────────────────────────────────────────────────────────
            ...
            foreach (ElementId id in selectedIds)
            {
                ...
                else if (element is RevitViewport viewport)
                {
                    ...
                    if (sheetFromViewport != null && !selectedSheets.Contains(sheetFromViewport))
                    {
                        selectedSheets.Add(sheetFromViewport);
                    }
                }
            }
