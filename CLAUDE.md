...
When a session starts in a worktree with an auto-generated branch name matching the pattern `claude\d?-\d+` (e.g. `claude3-152818212`, `claude-84866596`), **immediately rename the branch** to something descriptive based on the current task:

```bash
git branch -m claude3-152818212 fix-installer-partial-build
```

Do this before any other work so the PR and merge commit have a meaningful name.

## Worktree Cleanup (Agent Sessions)

**PROBLEM**: Claude agents run with their Bash tool CWD set to the worktree path. Once the worktree directory is deleted, ALL subsequent Bash commands fail with "Path does not exist" — even commands that use `cd /other/path &&` prefixes.

**SOLUTION**: Merge, remove worktree, and delete branch in a **single Bash call** before the directory is gone:
...
Run this as one command. Once it completes the agent's bash tool will error, but the work is done.

**If ExitWorktree tool is available** (only works when the worktree was created by `EnterWorktree` in the same session):
```
ExitWorktree(action: "remove")
```
This cleanly exits before the directory is touched.

**If bash is broken** (all commands fail with "Path does not exist") after the single-call cleanup, **do not assume the cleanup failed**. The merge, worktree removal, and branch deletion very likely all succeeded — git often cleans up the branch automatically as part of the worktree remove. Before asking the user to run anything manually, tell them to check `git log --oneline --decorate` and `git branch -v` to confirm the state. Only ask for manual intervention if something is genuinely missing.
